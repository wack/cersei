...
| `backend-vercel` |   | `VercelRuntime` — remote Firecracker microVMs on [Vercel Sandbox](https://vercel.com/docs/sandbox) via a built-in `reqwest` client. |
...
<Tabs items={['LocalProcessRuntime', 'DockerRuntime', 'SpritesRuntime', 'VercelRuntime']}>
...
<Tab value="VercelRuntime">

Remote isolation on [Vercel Sandbox](https://vercel.com/docs/sandbox) — Firecracker microVMs with persistent filesystems, snapshots, and pause/resume. Vercel ships no official Rust SDK, so this backend embeds a minimal `reqwest` client speaking the same REST API as the official `@vercel/sandbox` JS SDK. Behind the off-by-default `backend-vercel` feature.

```rust
let rt = VercelRuntime::from_env()?; // VERCEL_TOKEN / VERCEL_TEAM_ID / VERCEL_PROJECT_ID
let rt = VercelRuntime::new(
    VercelConfig::new(token, project_id)
        .with_team_id(team_id)
        .with_sandbox_timeout_ms(600_000),
)?;
```

Lifecycle mapping:

| Trait method | REST call |
|---|---|
| `create()` | `POST /v3/sandboxes` (`cpu_limit` → `resources.vcpus`, `env`/`labels` → `env`/`tags`) |
| `Commands::run()` / `stream()` | `POST /v2/sandboxes/sessions/<sid>/cmd` with `wait: true, logs: true` (ND-JSON stream) |
| `Filesystem::read` | `POST …/fs/read` (octet-stream) |
| `Filesystem::write` | `POST …/fs/write` (gzipped tar extracted at `x-cwd`) |
| `Filesystem::mkdir(recursive)` | `POST …/fs/mkdir` |
| `Filesystem::list/stat/remove` | shell commands, as in the docker backend |
| `snapshot()` | `POST …/sessions/<sid>/snapshot` + JSON manifest |
| `restore(&snap)` | `POST /v3/sandboxes` with `source: { type: "snapshot" }` — a fresh sandbox from the snapshot |
| `pause()` / `resume()` | `POST …/sessions/<sid>/stop` / `GET /v2/sandboxes/<name>?resume=true` (session id rotates) |
| `kill()` | stop session + `DELETE /v2/sandboxes/<name>` |

Backend-specific notes:

- Auth is the access-token flow: a Vercel access token plus team and project ids.
- `SandboxOpts::image` is forwarded only when it differs from the Cersei default; custom images must be Vercel managed images or live in Vercel Container Registry.
- `mem_limit` is ignored — Vercel scales memory with vCPUs.
- Volume mounts are rejected at `create()` (`shared_volumes: false`).
- `get()` falls back to attaching by sandbox name server-side, and `list()` queries the API with the `cersei-vm-` prefix.
